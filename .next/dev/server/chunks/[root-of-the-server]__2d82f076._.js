...
"[project]/Documents/GitHub/ThangBackend/pages/api/friends/list.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function handler(req, res) {
    ...
    try {
        ...
        const friendUids = friendList.map((f)=>f.uid);
        let hydratedFriends = [];
        if (friendUids.length > 0) {
            ...
            const friendsMap = new Map(friendsData.map((f)=>[
                    f._id,
                    f
                ]));
            // Fetch Party Info for friends in parties
            const partyIds = friendsData.map((f)=>f.partyId).filter((pid)=>pid).map((pid)=>new __TURBOPACK__imported__module__$5b$externals$5d2f$mongodb__$5b$external$5d$__$28$mongodb$2c$__cjs$2c$__$5b$project$5d2f$Documents$2f$GitHub$2f$ThangBackend$2f$node_modules$2f$mongodb$29$__["ObjectId"](pid));
            let partiesMap = new Map();
            if (partyIds.length > 0) {
                const partiesData = await db.collection("parties").find({
                    _id: {
                        $in: partyIds
                    }
                }, {
                ...
                partiesMap = new Map(partiesData.map((p)=>[
                        p._id.toString(),
                        p
                    ]));
            }
            ...
            hydratedFriends = friendList.map((f)=>{
                const data = friendsMap.get(f.uid);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2d82f076._.js.map