...
"[project]/Documents/GitHub/ThangBackend/pages/api/party/kick.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
    ...
    try {
        ...
        if (party.leaderUid !== uid) {
            return res.status(403).json({
                error: "Only the leader can kick members"
            });
        }
        // 5. Verify Target is in Party
        const isTargetInParty = party.members.some((m)=>m.uid === targetUid);
        if (!isTargetInParty) {
            return res.status(400).json({
                error: "Target user is not in the party"
            });
        }
        // 6. Kick User
        ...
        return res.status(200).json({
            message: "User kicked from party"
        });
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__32802729._.js.map