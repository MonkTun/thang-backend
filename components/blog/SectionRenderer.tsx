...
import type { Section, Block, BlockLayout } from "@/lib/blog/schema";
import { atomRegistry } from "@/lib/blog/atom-registry";
import { ROW_HEIGHT_PX } from "@/lib/blog/grid";
...
import { SectionVideoBackground } from "@/components/blog/SectionVideoBackground";
import {
  findMirror,
  useMirrorLibrary,
...
/**
 * Class + style for a section background. Image / video get
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          // A row is ROW_HEIGHT_PX unless a growing block (see
          // `blockSizingStyle`) needs more — then the rows it spans stretch
          // and everything below moves down with them.
...
function SingleBlock({
  ...
  // A mirror sizes like the block it renders.
  const contentType =
    block.type === "mirror"
      ? findMirror(mirrors, block.props.mirrorId)?.source.type
      : block.type;
  const grows = contentType ? !!atomRegistry[contentType].grows : false;
  return (
    <div
...
/**
 * Whether a block's content may size its rows. Rows are `minmax(16px, auto)`,
 * so by default any item taller than its span would stretch them — including
 * an `<img class="h-full">`, whose intrinsic height is the whole picture.
 * `contain: size` takes a block out of that calculation (it still fills its
 * box, and its children still render and overflow as before), which keeps
 * every fixed block exactly the size the editor drew. Blocks the registry
 * marks `grows` skip it, so their real height wins when it exceeds the box.
 */
export function blockSizingStyle(grows: boolean): React.CSSProperties {
  return grows ? {} : { contain: "size" };
}
...
export function SectionImageBackground({ bg }: { bg: Section["background"] }) {
  ...
  const image = isOptimizableImageSrc(bg.src) ? (
  ...
  return (
    <>
      {bg.parallax ? <ParallaxLayer>{image}</ParallaxLayer> : image}
      {bg.overlay > 0 && (
        <div
          aria-hidden
...
/** How far the parallax layer overhangs the section, top and bottom (px).
 *  Matches the `-inset-y-40` below; the drift is clamped to it so the image
 *  never runs out. */
const PARALLAX_OVERHANG_PX = 160;

/**
 * Wraps an image background so it drifts at a fraction of the scroll speed
 * (the home hero). The layer is taller than the section by the overhang on
 * each side, and the offset is measured from the section's own position, so
 * it works wherever the section sits on the page. Still under
 * prefers-reduced-motion, and in the editor canvas (which scrolls its own
...
function ParallaxLayer({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const drift = -section.getBoundingClientRect().top * 0.18;
      const clamped = Math.max(
        -PARALLAX_OVERHANG_PX,
        Math.min(PARALLAX_OVERHANG_PX, drift),
      );
  ...
  return (
    <div
      ref={ref}
      aria-hidden
      className="absolute inset-x-0 -inset-y-40 will-change-transform pointer-events-none"
    >
      {children}
    </div>
  );
