...
import type { TextProps } from "@/lib/blog/schema";
import { sanitizeRichText, sanitizeRichTextStrict } from "@/lib/blog/sanitize";
 ...
 * (use exactly one "h1" per post), displays the content as raw
 * HTML (sanitized — only <strong>/<em>/<br> survive; the server sanitizes
 * on write and read, and the public render re-applies the strict pass as a
 * last line of defence), and turns into a contentEditable rich-text editor
 * when an EditContext is present.
 *
...
export function Text(props: TextProps) {
  ...
  const className = cn(
    ...
    // The kicker renders as a <span>; without `block` its alignment is a no-op.
    "block outline-none"
  );
  ...
  if (!ctx) {
    return (
      <Tag
        className={className}
        style={style}
        dangerouslySetInnerHTML={{ __html: sanitizeRichTextStrict(content) }}
      />
    );
  }
