...
import type { ImageProps } from "@/lib/blog/schema";
...
/**
 * Whether a src can go through next/image's optimizer. Only same-origin
 * paths qualify: blog media lives in public/uploads/blog and is served as
 * `/uploads/blog/…`, and next.config.js allow-lists no remote hosts.
 * Everything else falls back to a raw <img>: the editor's manual-src input
 * streams partial strings ("h", "http…") and arbitrary external URLs into
 * atoms per keystroke, and next/image throws at render for both. SVGs are
 * excluded too (next/image refuses them by default and they gain nothing
 * from re-encoding).
 */
export function isOptimizableImageSrc(src: string): boolean {
  if (!src) return false;
  // Ignore any query string / hash when sniffing the extension.
...
export function imageTintMaskStyle({
  ...
  // Single-quote the URL so most safe filename characters survive; the
  // upload route (/api/admin/blog/upload) slugifies object names, so quotes
  // are not expected.
  const url = `url('${src}')`;
  const size =
    fit === "cover"
