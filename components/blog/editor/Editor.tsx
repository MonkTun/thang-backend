...
import { TagLibraryProvider } from "@/components/blog/TagLibraryContext";
import {
  MirrorLibraryProvider,
  findMirror,
} from "@/components/blog/MirrorLibraryContext";
import {
  atomRegistry,
  defaultsForBlock,
  isSourceBlockType,
} from "@/lib/blog/atom-registry";
import { nextFreeRow } from "@/lib/blog/rgl";
import {
  autoStackPage,
  autoStackSection,
  diffMobileLayout,
...
import type { SectionTemplate } from "@/lib/blog/section-templates";
import { readClipboard, writeClipboard } from "@/lib/blog/editor-clipboard";
import { uploadImageFile } from "@/lib/blog/upload-image";
import { adminFetch } from "@/lib/blog/admin-client";
import type { PostSummary } from "@/lib/blog/content";
import { pagePath, type ContentKind } from "@/lib/blog/site";
import { PostsProvider } from "@/components/blog/PostsContext";
...
/**
 * A site-config array the editor keeps a live working copy of (the tag
 ...
 * made elsewhere. We re-pull from the server whenever the tab regains focus
...
function useSiteLibrary<T>(
  ...
  // At most one POST per library is in flight; `dirty` marks a newer array
  // waiting to go out once it settles. Without this, two debounced saves
  // could reach the server out of order (a cold function is slower than a
  // warm one) and the older array would silently win.
  ...
  const flush = useCallback(() => {
    ...
    dirty.current = false;
    inFlight.current = adminFetch("/api/admin/blog/site", {
      method: "POST",
      ...
      .catch(() => {
        onError(`Failed to save the ${key} library`);
      })
      // Always continue the queue — the newest array goes out next.
      .then(flush);
  }, [key, onError]);
  ...
  useEffect(() => {
    const refresh = () => {
      // A local edit that is pending, queued or in flight must win over a
      // server pull, or the pull would revert it and the next edit would
      // build on (and re-save) the stale copy.
      if (saveTimer.current || inFlight.current || dirty.current) return;
...
type Props = {
  slug: string;
  /** Which collection `slug` belongs to — a blog post or a site page. */
  kind?: ContentKind;
  initialPage: Page;
  /** Link-picker entries: post slugs, plus site pages as "/path" hrefs. */
  availablePages?: string[];
  /** Published posts, newest first — what a Post grid block shows. */
  posts?: PostSummary[];
  /** Site-wide tag library — used by the Tags block + post meta tags. */
  initialTags?: TagDef[];
  /** Site-wide mirror library — sources for `mirror` blocks. */
...
export function Editor({
   ...
   * closure, but a pasted screenshot uploads in well under a second.
   */
  ...
  const save = useCallback(async () => {
    ...
    const posted = page;
    try {
      const res = await adminFetch("/api/admin/blog/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ slug, kind, page: posted }),
      });
      const body = (await res.json().catch(() => null)) as
        | { page?: Page; detail?: string; error?: string }
        | null;
      if (!res.ok) {
        throw new Error(body?.detail ?? body?.error ?? `HTTP ${res.status}`);
      }
      // The server validates AND sanitizes rich text, so adopt the page it
      // actually stored: the canvas and the dirty flag must reflect what the
      // public site will render, not what was posted. Edits made while the
      // request was in flight are kept (the working copy is only swapped
      // when it is still the object we posted).
      const canonical: Page = body?.page ?? posted;
      setSavedPage(canonical);
      setPage((cur) => (cur === posted ? canonical : cur));
      setStatus("saved");
      setTimeout(
      ...
      <div className="flex-1 grid grid-cols-[260px_minmax(0,1fr)_360px] min-h-0">
        ...
        <aside className="glass-panel border-l border-border overflow-y-auto">
          <PropertiesPanel
            ...
            // Same form as the picker's entries: pages are "/path" hrefs.
            currentSlug={kind === "page" ? pagePath(slug) : slug}
            siteTags={siteTags}
