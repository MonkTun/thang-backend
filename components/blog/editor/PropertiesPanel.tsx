...
import type { ContentKind } from "@/lib/blog/site";
import { SchemaForm } from "./SchemaForm";
import { SOCIAL_PLATFORM_LABELS } from "@/components/blog/atoms/SocialLinks";
...
import { VIDEO_MAX_MB } from "@/lib/blog/upload-limits";
import { adminFetch } from "@/lib/blog/admin-client";
import { sanitizeRichText } from "@/lib/blog/sanitize";
import { atomRegistry } from "@/lib/blog/atom-registry";
import { findMirror } from "@/components/blog/MirrorLibraryContext";
...
import {
  ...
  imageTintBgClass,
  imageTintLabel,
} from "@/components/blog/atoms/imageStyles";
import {
  hasMobileOverrides,
  mergeBlockForMobile,
  type Device,
} from "@/lib/blog/responsive";
import { isMobileOverridable } from "@/lib/blog/mobile-overrides";
...
type Props = {
  ...
  selection: Selection;
  device: Device;
  /** What is being edited — a blog post or a site page. Picks the meta
   *  panel's fields and copy. */
  kind?: ContentKind;
  /** Link-picker entries: post slugs, plus site pages as "/path" hrefs. */
  availablePages?: string[];
  /** The entry being edited (same form as above); excluded from the picker. */
  currentSlug?: string;
  /** Site-wide tag library — feeds the Tags block editor and the post's
   *  own tag suggestions. */
  siteTags?: TagDef[];
  onUpdateSiteTags?: (tags: TagDef[]) => void;
  /** Site-wide mirror library + its mutations. Sources are
...
/**
 * Page-level metadata — what the blog cards and the SEO tags read from
 * `page.meta`. A site page only needs the title / description / share
 * image; date, author and tags are post-only, so they are hidden there.
 */
function PageMeta({
  page,
  kind,
  ...
  page: Page;
  kind: ContentKind;
  /** Site-wide tag library — offered as suggestions for the post's tags. */
  siteTags?: TagDef[];
  onUpdate: (meta: Page["meta"]) => void;
}) {
  const meta = page.meta;
  const isPost = kind === "post";
  const patch = (p: Partial<Page["meta"]>) => onUpdate({ ...page.meta, ...p });
  return (
    <div className="p-5 space-y-5">
      <SectionHead title={isPost ? "Post" : "Page"} subtitle="Meta" />

      <Field label="status">
        <ToggleBtn
          label={meta.published ? "Published" : "Draft"}
          active={!!meta.published}
          onToggle={() => patch({ published: !meta.published })}
        />
        <p className="text-xs text-foreground/40 italic mt-1.5">
          {isPost
            ? "Published posts appear on /blog and the home page. Drafts are only reachable through Preview. Save to apply."
            : "A published page is live at its URL and listed in the sitemap. A draft is a 404 publicly and only reachable through Preview. Save to apply."}
        </p>
      </Field>
      ...
      <Field label="title">
        ...
        {!isPost && (
          <p className="text-xs text-foreground/40 italic mt-1.5">
            The browser tab and search-result title — it is not drawn on the
            page. Put the visible heading in a Text block.
          </p>
        )}
      </Field>
      <Field label="description">
        ...
        <p className="text-xs text-foreground/40 italic mt-1.5">
          {isPost
            ...
            : "Used as the meta description and the social share text."}
        </p>
      </Field>
      ...
      <Field label={isPost ? "cover image" : "share image"}>
        <ImageDialog
          value={meta.cover ?? ""}
          onChange={(cover) => patch({ cover })}
          label={isPost ? "cover image" : "share image"}
        />
        <p className="text-xs text-foreground/40 italic mt-1.5">
          {isPost
            ? "The card thumbnail and the social share image — 16:9 works best."
            : "The social share image. Empty uses the site default."}
        </p>
      </Field>
      <Field label={isPost ? "cover alt text" : "share image alt text"}>
        <input
          className={inputCls}
          value={meta.coverAlt ?? ""}
      ...
      <Field label="falling snow">
        <ToggleBtn
          label={meta.snow ? "On" : "Off"}
          active={!!meta.snow}
          onToggle={() => patch({ snow: !meta.snow })}
        />
        <p className="text-xs text-foreground/40 italic mt-1.5">
          A snow layer fixed behind the whole page; it shows through every
          section without an opaque background. Not drawn on this canvas —
          check it in Preview.
        </p>
      </Field>
      ...
      {isPost && (
        <>
          ...
          {/* Not a <Field>: that's a <label>, and the chips' remove buttons
              would fight the label's click forwarding. */}
          <div className="space-y-1.5">
            <span className="kicker block">tags</span>
            <PostTagsInput
              value={meta.tags ?? []}
              suggestions={siteTags.map((t) => t.name)}
...
/**
 * Chip input for the post's tags: comma or Enter adds, × removes,
 * Backspace on an empty draft pops the last one. Suggestions come from the
 * site-wide tag library (the Tags block's palette) so post tags and block
 * tags share one vocabulary, but any free text is accepted.
 */
function PostTagsInput({
  ...
  const has = (tags: string[], name: string) =>
  ...
  const commit = (raw: string) => {
    ...
    for (const name of names) {
      // Reuse the library's canonical casing when the name already exists.
      const canonical =
        suggestions.find((s) => s.toLowerCase() === name.toLowerCase()) ??
        name;
      if (!has(next, canonical)) next.push(canonical);
    }
    if (next.length !== value.length) onChange(next);
  };

  const unused = suggestions.filter((s) => !has(value, s));
  ...
  return (
    <div className="space-y-2">
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 rounded-full border border-accent/40 bg-accent/15 px-2.5 py-1 font-sans text-xs text-foreground"
            >
              {tag}
              <button
                type="button"
      ...
      <input
        className={inputCls}
        list={listId}
        placeholder={value.length ? "Add another…" : "devlog, patch-notes…"}
        value={draft}
        ...
        <div className="flex flex-wrap gap-1">
          {unused.map((s) => (
            <button
              ...
              type="button"
              onClick={() => commit(s)}
              className="kicker rounded-sm border border-dashed border-border px-2 py-1 text-foreground/60 hover:border-accent hover:text-accent transition-colors"
            >
              + {s}
            </button>
          ))}
      ...
      <p className="text-xs text-foreground/40 italic">
        Comma or Enter adds a tag. Suggestions come from the site tag
        library.
      </p>
    </div>
  );
...
/** Block types whose panel is generated from their zod schema (SchemaForm)
 *  instead of a hand-written form — the site-page blocks. */
const SCHEMA_FORM_BLOCKS = {
  heroTitle: heroTitlePropsSchema,
  postGrid: postGridPropsSchema,
  cards: cardsPropsSchema,
  footer: footerPropsSchema,
  fontSpecimen: fontSpecimenPropsSchema,
  swatches: swatchesPropsSchema,
} as const;
type SchemaFormBlockType = keyof typeof SCHEMA_FORM_BLOCKS;
...
function BlockTypeFields({
  ...
  return (
    <>
      ...
      {block.type in SCHEMA_FORM_BLOCKS && (
        <>
          <SchemaForm
            schema={SCHEMA_FORM_BLOCKS[block.type as SchemaFormBlockType]}
            value={block.props}
            onChange={(next) => onUpdate(next as Record<string, unknown>)}
          />
          {block.type === "postGrid" && (
            <Hint>
              Shows the newest published posts — there is nothing to add
              here. Count 0 lists every post.
            </Hint>
          )}
          {block.type === "footer" && (
            <Hint>
              Make this a mirror so every page shares one footer.
            </Hint>
          )}
        </>
      )}
      {block.type === "quote" && (
        <>
          <Field label="quote (strong / em / br only)">
            <textarea
              rows={4}
              className={inputCls}
              value={(block.props as { quote: string }).quote}
              onChange={(e) => onUpdate({ quote: e.target.value })}
              // Same whitelist the server enforces on save — normalise here
              // so the canvas shows what will actually be stored.
              onBlur={(e) => {
                const clean = sanitizeRichText(e.target.value);
                if (clean !== e.target.value) onUpdate({ quote: clean });
              }}
            />
          </Field>
          <Field label="attribution">
            <input
              ...
              onBlur={(e) => {
                const clean = sanitizeRichText(e.target.value);
                if (clean !== e.target.value) onUpdate({ attribution: clean });
              }}
            />
          </Field>
        </>
...
function TextBlockProps({
  ...
  return (
    <>
      <Field label="content (HTML: strong / em / br only)">
        <textarea
          rows={5}
          className={cn(inputCls, "font-sans text-xs")}
          value={props.content}
          onChange={(e) => onUpdate({ content: e.target.value })}
          // Same whitelist the server enforces on save — normalise here so
          // the canvas shows what will actually be stored.
          onBlur={(e) => {
            const clean = sanitizeRichText(e.target.value);
            if (clean !== e.target.value) onUpdate({ content: clean });
          }}
        />
      </Field>
...
function ImageLibrary({
  ...
  useEffect(() => {
    let cancelled = false;
    setError(null);
    adminFetch("/api/admin/blog/uploads?folder=images")
      .then(async (res) => {
    ...
    <div className="space-y-4">
      ...
      {bg.type === "image" && (
        <>
          ...
          <Field label="fade into page">
            <ToggleBtn
              label={bg.fadeBottom ? "On" : "Off"}
              active={!!bg.fadeBottom}
              onToggle={() => onChange({ ...bg, fadeBottom: !bg.fadeBottom })}
            />
            <p className="text-xs text-foreground/40 italic">
              Melts the bottom of the image into the page background, so the
              next section starts without a hard edge.
            </p>
          </Field>

          <Field label="parallax">
            <ToggleBtn
              label={bg.parallax ? "On" : "Off"}
              active={!!bg.parallax}
              onToggle={() => onChange({ ...bg, parallax: !bg.parallax })}
            />
            <p className="text-xs text-foreground/40 italic">
              The image drifts slower than the page scrolls. Public page and
              Preview only — the canvas keeps it still.
            </p>
          </Field>
        </>
      )}
      ...
      {bg.type === "video" && (
        <>
          <Field label="source">
            <SegmentBar
              options={["youtube", "file"]}
              labels={{ youtube: "YouTube", file: "Upload" }}
              value={bg.source ?? "youtube"}
              onChange={(v) =>
                onChange({ ...bg, source: v as "youtube" | "file" })
              }
            />
          </Field>

          {(bg.source ?? "youtube") === "youtube" && (
          ...
          {bg.source === "file" && (
            <>
              <Field label="video file">
                <VideoUploader
                  src={bg.src ?? ""}
                  onUploaded={(src) => onChange({ ...bg, src })}
                />
              </Field>
              <Field label="src (manual URL)">
                <input
                  className={cn(inputCls, "font-sans text-xs")}
                  placeholder="https://…"
                  value={bg.src ?? ""}
          ...
          <Field label={`speed — ${bg.playbackRate}×`}>
            ...
            <p className="text-xs text-foreground/40 italic mt-1.5">
              ...
              snapped to the nearest one. Uploaded files honor the same
              set.
            </p>
          </Field>
...
/** File name of an image src, for labels and derived names. Uploads are
 *  plain `/uploads/blog/images/<name>` paths; external URLs may carry a
 *  query string or a URL-encoded path, so strip / decode defensively. */
function srcFileName(src: string): string {
  const noQuery = src.split(/[?#]/)[0];
  const last = noQuery.split("/").pop() ?? "";
  try {
    return decodeURIComponent(last).split("/").pop() ?? last;
  } catch {
    return last;
  }
}

/** Sources the crop tool can draw onto a canvas without tainting it:
 *  same-origin paths only (uploads live in public/uploads/blog). Arbitrary
 *  third-party URLs stay read-only. */
function isCroppableSrc(src: string): boolean {
  return src.startsWith("/") && !src.startsWith("//");
}

/**
 * Destructive crop: drag a rectangle over the image (aspect presets or
 * freeform), Apply bakes that region to a canvas at the source's native
 * resolution and saves the result as a new upload — the original stays in
 * the library — then selects the copy. Same-origin sources only: a
 * cross-origin image would taint the canvas and the export would throw.
 */
function CropTool({
  src,
  onResult,
  ...
  const local = isCroppableSrc(src);
  ...
  async function apply() {
    ...
    try {
      ...
      // PNG sources keep PNG (alpha survives); everything else re-encodes
      // to WebP like the uploader does.
      const fileName = srcFileName(src);
      ...
      const stored = await uploadMedia(file, "images");
      onResult(stored.url);
      setActive(false);
    } catch (err) {
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-foreground/40 italic">
          {local
            ...
            : "Only uploaded images (or same-site paths) can be cropped."}
        </p>
      </div>
    );
  }
...
/* ---------------- shared image dialog ----------------
   One popup used by the Image block, section background, carousel items and
   the post cover. Bundles upload + library pick + manual URL + crop + preview
...
function Modal({
  ...
  return createPortal(
    <div
      // Portalled to <body>, outside the editor's `tw-reset` root — the
      // scoped preflight must be re-applied here or the dialog gets UA styles.
      className="tw-reset fixed inset-0 z-[100] flex items-center justify-center p-4"
      role="dialog"
      aria-modal="true"
      aria-label={title}
    >
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block truncate text-sm text-foreground">
            {value ? srcFileName(value) : `Choose ${label}`}
          </span>
...
/**
 * Click-to-pick + drag-and-drop image uploader. Goes through
 * `uploadImageFile` (browser-side downscale, then public/uploads/blog/images)
 * and hands back the public path.
 */
...
/**
 * Click-to-pick + drag-and-drop video uploader — the ImageUploader's
 * sibling. Goes through `uploadVideoFile` (no browser-side transcoding:
 * videos are stored as-is in public/uploads/blog/videos, so keep them
 * web-ready mp4 / webm). They are committed with the post like any other
 * file, so keep them short — prefer YouTube for long videos.
 */
function VideoUploader({
  ...
  async function uploadFile(rawFile: File) {
    ...
    setError(null);
    try {
      onUploaded(await uploadVideoFile(rawFile));
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  return (
    <div className="space-y-2">
      {/* The preview sits OUTSIDE the <label> so its native controls stay
          usable — inside, every click would forward to the file input. */}
      {src && (
        <video
          key={src}
          src={src}
          ...
          playsInline
          preload="metadata"
          className="block w-full max-h-40 rounded-sm border border-border bg-surface object-contain"
        />
      )}
      <label
        onDragOver={(e) => {
      ...
      >
        <span className="kicker text-foreground/60 pointer-events-none">
          {src ? "Replace video" : "No video yet"}
        </span>
        <span className="mt-2 text-xs text-foreground/60 italic pointer-events-none">
          {busy
      ...
      <p className="text-xs text-foreground/40 italic">
        mp4 / webm, up to {VIDEO_MAX_MB} MB. Saved to public/uploads/blog
        and committed with the post — prefer YouTube for long videos.
      </p>
    </div>
  );
}
      ...
      {source === "file" && (
        <>
          ...
          <Field label="poster (optional)">
            <ImageDialog
              value={props.poster ?? ""}
              onChange={(poster) => onUpdate({ poster })}
              label="poster"
            />
            <p className="text-xs text-foreground/40 italic mt-1.5">
              Shown before playback starts and while the video loads.
            </p>
          </Field>
        </>
      )}
    ...
    <>
      ...
      <Field label="href">
        <input
          ...
          placeholder="/download, https://…, #section-id"
        />
      </Field>
      <Field label="variant">
        <SegmentBar
...
/** Static site routes the link picker offers ahead of the posts. */
// Coded routes only — site pages ("/", "/download", …) arrive through
// `availablePages` as "/path" entries.
const STATIC_LINKS = ["/blog"];

/** Dropdown of the site's routes + posts that writes a normalized href on
 *  pick. Shared between the button block and carousel item link fields; the
...
function PageLinkPicker({
  ...
  // Static routes are already hrefs; post slugs live under /blog/.
  const slugToHref = (s: string) => (s.startsWith("/") ? s : `/blog/${s}`);

  const pickablePages = availablePages.filter((s) => s !== currentSlug);

  // What's currently selected in the picker, derived from href by exact
  // match (a nested slug's href can't collide with its parent's).
  const pickerValue: string =
    [...STATIC_LINKS, ...pickablePages].find((s) => slugToHref(s) === href) ??
    "";
  ...
  return (
    ...
    >
      ...
      <optgroup label="Site">
        {[...STATIC_LINKS, ...pickablePages.filter((s) => s.startsWith("/"))].map(
          (path) => (
      ...
      {pickablePages.some((s) => !s.startsWith("/")) && (
