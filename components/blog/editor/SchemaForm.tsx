 ...
 *
 * Supports: string, number, boolean, enum, array, object, optional and
 * default. New shapes (unions) can be added when blocks need them.
 */
...
function Field({ name, schema, value, onChange, depth }: FieldProps) {
  ...
  // Default → same: the block already carries a value for every key.
...
export function defaultsFor(schema: AnyZod): unknown {
  ...
  // A new array item starts from the schema's own defaults.
