...
import type { Device } from "@/lib/blog/responsive";
import { previewPath, publicPath, type ContentKind } from "@/lib/blog/site";

type Status = "idle" | "saving" | "saved" | "error";

type Props = {
  slug: string;
  /** Blog post or site page — picks the labels, the preview route and the
   *  file the save hint points at. */
  kind: ContentKind;
  /** `page.meta.published` — drives the status pill next to the slug. */
  published: boolean;
  dirty: boolean;
  status: Status;
  errorMessage?: string;
...
export function Toolbar({
  ...
  // Preview goes through the admin-only draft route (it renders the saved
  // page whether or not it's published, so drafts can be checked before
  // going live). Per-segment encoding keeps nested slugs like
  // devlog/week-1 routable.
  const baseHref = previewPath(kind, slug);
  const isPage = kind === "page";
  const previewHref = previewAnchor ? `${baseHref}#${previewAnchor}` : baseHref;

  // If there are unsaved changes, the preview route would render stale JSON
  ...
  // background and reload the tab once the save has landed.
  ...
  return (
    <header className="h-14 shrink-0 glass-strong border-b border-border flex items-center px-4 gap-4 z-50">
      ...
      <div className="kicker text-foreground/60">{publicPath(kind, slug)}</div>
      <span
        title={
          published
            ? isPage
              ? "Live at its URL once committed and pushed"
              : "Visible on /blog and the home page"
            : `Only visible through Preview until published (${isPage ? "Page" : "Post"} panel)`
        }
        className={cn(
          "kicker px-2 py-0.5 rounded-full border",
          published
            ? "border-accent/40 bg-accent/15 text-accent"
            : "border-border bg-background/40 text-foreground/50"
        )}
      >
        {published ? "Published" : "Draft"}
      </span>
      <span
        className="kicker text-foreground/40 hidden lg:inline"
        title={`Save writes ${isPage ? "content/pages" : "content/blog/posts"}/${slug}.json. The live site updates when that file is committed and pushed.`}
      >
      ...
      <Link
        ...
        title={dirty ? "Saves first, then opens" : "Open draft preview"}
      >
        Preview ↗
      </Link>
